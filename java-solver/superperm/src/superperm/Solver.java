...
public class Solver {
    ...
    public static void solve(PrintStream out, int n, int[] next, long[][] chain, int[] initialInserts, int maxInsert) {
        int nm1 = n - 1, nm2 = n - 2;
        int total = next.length;
        int bitSize = chain[0].length;
        int groupSize = nm2 * (n - 3);
        if (maxInsert <= 0)
            maxInsert = (total / n / nm2 - nm1);
        int minInsert, insertCur = 0;
        ...
        for (int i = 0; i < n * nm1 * nm2; i++) {
            if (!intersect(used, chain[i]))
                queue[queueWrite++] = i;
        }
        if (initialInserts != null) {
            for (int value : initialInserts) {
                while (queue[queueRead++] != value);
        ...
        minInsert = insertCur;
        System.out.println("Adding " + maxInsert + " cycles");
